...
reg     [63:0]  mrd_addr;
reg     [10:0]  mrd_length;
reg     [10:0]  mrd_length_ff;
wire    [9:0]   mrd_length_tx;
wire    [10:0]  mrd_length_tx_dw;
...
assign mrd_length_tx_dw = (mrd_length_ff > max_rd_req_size) ? max_rd_req_size : mrd_length_ff;
assign mrd_length_tx = (mrd_length_tx_dw == 11'd1024) ? 10'd0 : mrd_length_tx_dw[9:0];
...
begin
    if(!rst_n)
        ...
        mrd_addr <= i_req_addr;
    else if(mrd_length != 11'd0 && tx_done)
        mrd_addr <= mrd_addr + {51'b0,max_rd_req_size,2'b0};
end
...
begin
    if(!rst_n)
        ...
        tx_mrd <= 1'b1;
    else if(mrd_length != 11'd0 && tx_done && ~mrd_tx_halt)  //for length > max_rd_req_size
        tx_mrd <= 1'b1;
end
...
begin
    if(!rst_n)
        mrd32_req_tx <= 1'b0;
    else if(tx_done && (mrd_length == 11'd0))
        mrd32_req_tx <= 1'b0;
