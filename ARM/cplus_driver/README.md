...
HDMI overlays are rendered in normal cached memory before a sequential copy to the DRM framebuffer. `--display-mask 0` remains available when a fixed mask is needed only for decisions, while stored-video playback keeps the mask enabled for visual verification.
