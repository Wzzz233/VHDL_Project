...
/* Submit the newest immutable frame. A queued older frame is displaced. */
int lpr_infer_submit_latest(struct infer_state *st,
                            const struct lpr_frame_ref *frame);

/* Invalidate queued/in-flight work from older source generations. */
void lpr_infer_reset(struct infer_state *st, uint64_t source_generation);

/* Snapshot the newest accepted result. */
bool lpr_infer_get_result(struct infer_state *st, struct live_result *res);

/* Generation-checked publication entry point, exposed for race testing. */
