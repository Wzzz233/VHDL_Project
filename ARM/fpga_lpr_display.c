...
static void estimate_quad_rect_size(const float quad[8], int *out_w, int *out_h);
static bool warp_quad_to_rect_exact_rgb888(const uint8_t *rgb, int img_w, int img_h,
                                           const float quad_in[8], uint8_t *dst,
                                           int dst_cap_w, int dst_cap_h,
                                           int *out_w, int *out_h);
static bool fit_line_y_from_x(const struct point2f *pts, int n, float *m, float *b);
static bool fit_line_x_from_y(const struct point2f *pts, int n, float *m, float *b);
static bool refine_warped_plate_quad_rgb888(const uint8_t *rgb, int w, int h, float quad_out[8]);
...
static bool fit_line_y_from_x(const struct point2f *pts, int n, float *m, float *b)
{
    ...
    for (i = 0; i < n; i++) {
        sx += pts[i].x;
        sy += pts[i].y;
        sxx += (double)pts[i].x * (double)pts[i].x;
        sxy += (double)pts[i].x * (double)pts[i].y;
    }
    den = (double)n * sxx - sx * sx;
    if (fabs(den) < 1e-8)
        return false;
    *m = (float)(((double)n * sxy - sx * sy) / den);
    *b = (float)((sy - (double)(*m) * sx) / (double)n);
    return true;
}

static bool fit_line_x_from_y(const struct point2f *pts, int n, float *m, float *b)
{
    double sx = 0.0, sy = 0.0, syy = 0.0, sxy = 0.0;
    double den;
    int i;
    if (!pts || n < 6 || !m || !b)
        return false;
    for (i = 0; i < n; i++) {
        sx += pts[i].x;
        sy += pts[i].y;
        syy += (double)pts[i].y * (double)pts[i].y;
        sxy += (double)pts[i].x * (double)pts[i].y;
    }
    den = (double)n * syy - sy * sy;
    if (fabs(den) < 1e-8)
        return false;
    *m = (float)(((double)n * sxy - sx * sy) / den);
    *b = (float)((sx - (double)(*m) * sy) / (double)n);
    return true;
}

static inline int plate_green_score_at(const uint8_t *rgb, int w, int h, int x, int y)
{
    const uint8_t *p;
    if (x < 0) x = 0;
    if (x >= w) x = w - 1;
    if (y < 0) y = 0;
    if (y >= h) y = h - 1;
    p = rgb + ((size_t)y * (size_t)w + (size_t)x) * 3U;
    return (int)p[1] * 2 - (int)p[0] - (int)p[2];
}

static bool refine_warped_plate_quad_rgb888(const uint8_t *rgb, int w, int h, float quad_out[8])
{
    struct point2f top_pts[96];
    struct point2f bottom_pts[96];
    struct point2f left_pts[96];
    struct point2f right_pts[96];
    struct point2f top_a, top_b, bot_a, bot_b, left_a, left_b, right_a, right_b;
    struct point2f tl, tr, br, bl;
    float mt, bt, mb, bb, ml, bl0, mr, br0;
    int n_top = 0, n_bottom = 0, n_left = 0, n_right = 0;
    int step_x, step_y;
    int x, y;

    if (!rgb || !quad_out || w < 24 || h < 12)
        return false;
    ...
    for (x = step_x; x < w - step_x && n_top < 96 && n_bottom < 96; x += step_x) {
        int best_top_y = -1, best_bottom_y = -1;
        int best_top_s = 0, best_bottom_s = 0;
        for (y = 1; y < (h * 45) / 100; y++) {
            int s = plate_green_score_at(rgb, w, h, x, y + 1) - plate_green_score_at(rgb, w, h, x, y - 1);
            if (s > best_top_s) {
                best_top_s = s;
                best_top_y = y;
            }
        }
        for (y = h - 2; y >= (h * 55) / 100; y--) {
            int s = plate_green_score_at(rgb, w, h, x, y - 1) - plate_green_score_at(rgb, w, h, x, y + 1);
            if (s > best_bottom_s) {
                best_bottom_s = s;
                best_bottom_y = y;
        ...
        if (best_top_y >= 0 && best_top_s > 12) {
            top_pts[n_top].x = (float)x;
            top_pts[n_top].y = (float)best_top_y;
            n_top++;
        }
        if (best_bottom_y >= 0 && best_bottom_s > 12) {
            bottom_pts[n_bottom].x = (float)x;
            bottom_pts[n_bottom].y = (float)best_bottom_y;
            n_bottom++;
        }
    }

    for (y = step_y; y < h - step_y && n_left < 96 && n_right < 96; y += step_y) {
        int best_left_x = -1, best_right_x = -1;
        int best_left_s = 0, best_right_s = 0;
        for (x = 1; x < (w * 40) / 100; x++) {
            int s = plate_green_score_at(rgb, w, h, x + 1, y) - plate_green_score_at(rgb, w, h, x - 1, y);
            if (s > best_left_s) {
                best_left_s = s;
                best_left_x = x;
            }
        }
        for (x = w - 2; x >= (w * 60) / 100; x--) {
            int s = plate_green_score_at(rgb, w, h, x - 1, y) - plate_green_score_at(rgb, w, h, x + 1, y);
            if (s > best_right_s) {
                best_right_s = s;
                best_right_x = x;
        ...
        if (best_left_x >= 0 && best_left_s > 12) {
            left_pts[n_left].x = (float)best_left_x;
            left_pts[n_left].y = (float)y;
            n_left++;
        }
        if (best_right_x >= 0 && best_right_s > 12) {
            right_pts[n_right].x = (float)best_right_x;
            right_pts[n_right].y = (float)y;
            n_right++;
        }
    }

    if (!fit_line_y_from_x(top_pts, n_top, &mt, &bt) ||
        !fit_line_y_from_x(bottom_pts, n_bottom, &mb, &bb) ||
        !fit_line_x_from_y(left_pts, n_left, &ml, &bl0) ||
        !fit_line_x_from_y(right_pts, n_right, &mr, &br0))
        return false;

    if (fabsf(mt) > 0.35f || fabsf(mb) > 0.35f || fabsf(ml) > 0.35f || fabsf(mr) > 0.35f)
        return false;

    top_a.x = 0.0f;        top_a.y = bt;
    top_b.x = (float)(w - 1); top_b.y = mt * (float)(w - 1) + bt;
    bot_a.x = 0.0f;        bot_a.y = bb;
    bot_b.x = (float)(w - 1); bot_b.y = mb * (float)(w - 1) + bb;
    left_a.x = bl0;        left_a.y = 0.0f;
    left_b.x = ml * (float)(h - 1) + bl0; left_b.y = (float)(h - 1);
    right_a.x = br0;       right_a.y = 0.0f;
    right_b.x = mr * (float)(h - 1) + br0; right_b.y = (float)(h - 1);

    tl = line_intersection(&top_a, &top_b, &left_a, &left_b);
    tr = line_intersection(&top_a, &top_b, &right_a, &right_b);
    br = line_intersection(&bot_a, &bot_b, &right_a, &right_b);
    bl = line_intersection(&bot_a, &bot_b, &left_a, &left_b);

    quad_out[0] = tl.x; quad_out[1] = tl.y;
    quad_out[2] = tr.x; quad_out[3] = tr.y;
    quad_out[4] = br.x; quad_out[5] = br.y;
    quad_out[6] = bl.x; quad_out[7] = bl.y;
    clip_quad_to_image(quad_out, w, h, quad_out);

    {
        float width_top = hypotf(quad_out[2] - quad_out[0], quad_out[3] - quad_out[1]);
        float width_bottom = hypotf(quad_out[4] - quad_out[6], quad_out[5] - quad_out[7]);
        float height_left = hypotf(quad_out[6] - quad_out[0], quad_out[7] - quad_out[1]);
        float height_right = hypotf(quad_out[4] - quad_out[2], quad_out[5] - quad_out[3]);
        float avg_w = 0.5f * (width_top + width_bottom);
        float avg_h = 0.5f * (height_left + height_right);
        if (avg_w < (float)w * 0.45f || avg_h < (float)h * 0.45f)
            return false;
        if (avg_h <= 1.0f || avg_w / avg_h < 2.0f || avg_w / avg_h > 8.5f)
            return false;
    }
    return true;
}
                                           ...
                                           int *out_w, int *out_h)
{
    ...
    estimate_quad_rect_size(quad, &dw, &dh);
    ...
    *used_obb_warp = false;
    ...
    if (ctx->opt.ocr_crop_mode == OCR_CROP_OBB_WARP && plate_box->has_obb) {
        ...
        if (warp_quad_to_rect_rgb888(rgb, img_w, img_h, ordered,
                                     crop_buf, crop_cap_w, crop_cap_h, 0.0f, used_quad,
                                     crop_w, crop_h)) {
            float refined_quad[8];
            if (refine_warped_plate_quad_rgb888(crop_buf, *crop_w, *crop_h, refined_quad)) {
                int exact_w;
                int exact_h;
                uint8_t *tmp_buf;
                estimate_quad_rect_size(refined_quad, &exact_w, &exact_h);
                tmp_buf = malloc((size_t)exact_w * (size_t)exact_h * 3U);
                if (tmp_buf) {
                    int final_w;
                    int final_h;
                    if (warp_quad_to_rect_exact_rgb888(crop_buf, *crop_w, *crop_h,
                                                       refined_quad, tmp_buf, exact_w, exact_h,
                                                       &final_w, &final_h)) {
                        memcpy(crop_buf, tmp_buf, (size_t)final_w * (size_t)final_h * 3U);
                        *crop_w = final_w;
                        *crop_h = final_h;
                    }
                    free(tmp_buf);
...
static int run_offline_once(struct app_ctx *ctx)
{
    ...
    if (ctx->opt.ocr_min_occ_ratio > 0.0f &&
        ...
        if (used_obb_warp && pd.box.has_obb) {
            float used_quad[8];
            if (warp_quad_to_rect_rgb888(rgb, w, h, pd.box.quad,
                                         plate_crop, w, h, 0.08f, used_quad,
                                         &new_w, &new_h)) {
                pd.crop_box = pd.box;
                bbox_from_quad(used_quad, w, h, &pd.crop_box);
                crop_w = new_w;
                crop_h = new_h;
                occ_ratio = estimate_ocr_occ_ratio(ctx, crop_w, crop_h);
                have_recap = true;
        ...
        if (!have_recap) {
            ...
            if (!prepare_plate_crop_rgb888(ctx, rgb, w, h, &pd.box,
                                           ...
                                           &occ_ratio, &used_obb_warp))
                goto out;
            fprintf(stderr, "[ocr-recrop] trigger=0 old_occ=%.3f new_mode=%s reason=not-improved\n",
                    old_occ, mode_tag);
        }
    }
    ...
    while (ctx->running) {
        ...
        for (i = 0; i < stable_plate_count && r.plate_count < MAX_DETS; i++) {
            ...
            if (ctx->opt.ocr_min_occ_ratio > 0.0f &&
                ...
                if (used_obb_warp && pd.box.has_obb) {
                    float used_quad[8];
                    if (warp_quad_to_rect_rgb888(rgb_full, (int)ctx->frame_width, (int)ctx->frame_height, pd.box.quad,
                                                 plate_crop, (int)ctx->frame_width, (int)ctx->frame_height,
                                                 0.08f, used_quad, &new_w, &new_h)) {
                        pd.crop_box = pd.box;
                        bbox_from_quad(used_quad, (int)ctx->frame_width, (int)ctx->frame_height, &pd.crop_box);
                        crop_w = new_w;
                        crop_h = new_h;
                        occ_ratio = estimate_ocr_occ_ratio(ctx, crop_w, crop_h);
                        have_recap = true;
